...
class ImportJsonDialog: BaseDialogFragmentView<ImportJsonContract.View, ImportJsonContract.Presenter>(),
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        dialogView.findViewById<Button>(R.id.import_file_browse_button).setOnClickListener { (activity as MainActivity).selectExternalFile("image/*", RequestCodes.READ_EXTERNAL_IMAGE.value)}
    ...
    private fun emptyTextCheck(v: EditText, hasFocus: Boolean) {
        ...
        } else if (v.text.trim().isNotEmpty()) {
            v.error = null
        }
    }

    private fun returnValues(dialog: DialogInterface) {
        val nameView = dialogView.findViewById<EditText>(R.id.pattern_name)
    ...
    fun setFilenameViewText(uri: Uri) {
        ...
        (dialogView.findViewById(R.id.file_url_edittext) as EditText).hint = fileName
        (dialogView.findViewById(R.id.pattern_name) as EditText).hint = fileName.split(Regex("\\..+$"))[0]
    }
}
